...
export class QPROPActor2 extends Actor{
    //Spiders.js related
    thisDir
    ...
    parentRefs                  : Array<FarRef<QPROPActor2>>
    childRefs                   : Array<FarRef<QPROPActor2>>
    ...
    init(){
        ...
        this.childTypes.forEach((childType : PubSubTag)=>{
            this.psClient.subscribe(childType).each((childRef : FarRef<QPROPActor2>)=>{
        ...
        this.parentTypes.forEach((parentType : PubSubTag)=>{
            this.psClient.subscribe(parentType).each((parentRef : FarRef<QPROPActor2>)=>{
    ...
    getSources(sources : Array<PubSubTag>,initProp : PropagationValue){
        ...
        if(this.gotAllSources()){
            ...
            if(this.amSink()){
                let send = ()=>{
                    this.parentRefs.forEach((ref : FarRef<QPROPActor2>)=>{
                        ref.getStart()
                    })
                }
                ...
                let send = ()=>{
                    this.childRefs.forEach((ref : FarRef<QPROPActor2>)=>{
        ...
        if(signal.id == this.publishedSignalId){
            if(this.ready){
                ...
                this.sendToAllChildren(()=>{
                    this.childRefs.forEach((child : FarRef<QPROPActor2>)=>{
    ...
    newChild(childType : PubSubTag,childRef : FarRef<QPROPActor2>) : Promise<Array<any>>{
    ...
    addDependency(toParent : PubSubTag){
        if(this.inChange){
            ...
            this.inChange = true
            this.psClient.subscribe(toParent).once((newParentRef : FarRef<QPROPActor2>)=>{
    ...
    addSources(from : PubSubTag,sources : Array<string>) : Promise<any>{
        ...
        let childPromises = this.childRefs.map((child : FarRef<QPROPActor2>)=>{return child.addSources(this.ownType,sources)})
