...
let getAllData = (prefix, arrayIndex, fileIndex, resolver, valuesArray, errorArray) => {
    return new Promise((resolve) => {
        let stream;
        if (fileIndex == 0) {
            stream = fss.createReadStream("../UseCase/Bertha/Regular/Latency/" + prefix + "2.csv");
        }
        else {
            stream = fss.createReadStream("../UseCase/Bertha/Regular/Latency/" + prefix + fileIndex + ".csv");
        }
        let allData = [];
        var csvStream = csv()
