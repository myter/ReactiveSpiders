...
class ServiceInfo {
    constructor(tag, parents, children) {
        this.tag = tag;
        ...
        let idle = () => {
            ...
            if (valsReceived > 0) {
                ...
                if (valsReceived == totalVals) {
                    ...
                    this.memWriter.end();
                    if (changes > 0) {
                        averageMem(csvFileName, changes, "Admitter");
                    }
                    else {
...
class SourceService extends MicroService_1.MicroServiceApp {
    constructor(isQPROP, rate, totalVals, csvFileName, myAddress, myPort, monitorIP, monitorPort, myTag, directParentsTags, directChildrenTags, dynamicLinkTags, changes) {
...
class DerivedService extends MicroService_1.MicroServiceApp {
    constructor(isQPROP, rate, totalVals, csvFileName, myAddress, myPort, monitorIP, monitorPort, myTag, directParentsTag, directChildrenTags, changes) {
        super(myAddress, myPort, monitorIP, monitorPort);
        this.close = false;
        this.rate = rate;
        this.changes = changes;
        ...
        this.lift((args) => {
            ...
            if (valsReceived == totalVals) {
                ...
                if (changes > 0) {
                    ...
                    averageMem(csvFileName, changes, myTag.tagVal);
                }
                else {
...
//# sourceMappingURL=ServicesJane.js.map